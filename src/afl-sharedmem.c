...
#ifdef QMSAN
//qmsan shared memory cleaning
void msan_shm_deinit(sharedmem_t *shm) {

  if (shm == NULL) { return; }

  unsetenv(MSAN_SHM_ENV_VAR);

  shmctl(shm->shm_id, IPC_RMID, NULL);

  shm->map = NULL;

  OKF("[QMSAN] shmem destroyed with id: %d", shm->shm_id);

}

//qmsan shared memory creation
u8 *msan_shm_init(sharedmem_t* shmem) {
  ...
  if (id < 0) {
    ...
    PFATAL("[QMSAN]shmget() failed, try running afl-system-config");

  }

  shm_str = alloc_printf("%d", id);

  setenv(MSAN_SHM_ENV_VAR, shm_str, 1);
  ...
  if (ptr == (void *)-1 || !ptr)
    PFATAL("[QMSAN]shmat() failed");

  shmem->shm_id = id;
  shmem->map = ptr;

  OKF("[QMSAN] shmem created with id: %d", id);
